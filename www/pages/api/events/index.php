<?php
    ...
    if( isset($url[1]) && !empty($url[1])){
        $query = "SELECT * FROM `events` WHERE `event-id` = '".$url[1]."' LIMIT 1;";
        ...
        if( empty( $event ) ){
            ...
            $result=array_merge($result,array("success" => false ) );
            $result=array_merge($result,array("message" => "Unknown event in DB" ) );
    ...
    $path = explode( "/events/", utf8_decode(urldecode($_SERVER['REQUEST_URI'])) );

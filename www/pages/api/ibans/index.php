<?php
    ...
    // S'il n'y a qu'un seul iban recherché.
    if( isset($url[1]) && !empty($url[1])){
        $query = "SELECT * FROM `ibans` WHERE `iban` = '".$url[1]."' LIMIT 1;";
        ...
        $iban = mysqli_fetch_array($ibans,MYSQLI_ASSOC);

        if( empty($iban ) ){
            header("HTTP/1.1 404 Not Found");
            $result=array_merge($result,array("success" => false ) );
            $result=array_merge($result,array("message" => "Unknown iban in DB" ) );
        ...
        $result=array_merge($result,$iban );
        echo( json_encode($result) );
        return;
    }

    $query = 'SELECT * FROM `ibans`';
    $path = explode( "/ibans/", utf8_decode(urldecode($_SERVER['REQUEST_URI'])) );
    ...
    echo( json_encode( mysqli_fetch_all($ibans,MYSQLI_ASSOC) ) );

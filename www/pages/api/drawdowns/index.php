<?php
    ...
    // S'il n'y a qu'un seul drawdown recherché.
    if( isset($url[1]) && !empty($url[1])){
        $query = "SELECT * FROM `drawdowns` WHERE `drawdown-id` = '".$url[1]."' LIMIT 1;";
        ...
        $drawdown = mysqli_fetch_array($drawdowns,MYSQLI_ASSOC);

        if (empty($drawdown)) {
            header("HTTP/1.1 404 Not Found");
            $result=array_merge($result,array("success" => false ) );
            $result=array_merge($result,array("message" => "Unknown drawdown in DB" ) );
        ...
        $result=array_merge($result,$drawdown );
        echo( json_encode($result) );
        return;
    }

    $query = 'SELECT * FROM `drawdowns`';
    $path = explode( "/drawdowns/", utf8_decode(urldecode($_SERVER['REQUEST_URI'])) );
    ...
    echo( json_encode( mysqli_fetch_all($drawdowns,MYSQLI_ASSOC) ) );

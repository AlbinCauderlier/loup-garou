<?php
    ...
    if( isset($url[1]) && !empty($url[1])){
        $query = "SELECT * FROM `invoice_lines` WHERE `invoice-line-id` = '".$url[1]."' LIMIT 1;";
        ...
        $invoice_line = mysqli_fetch_array($invoice_lines,MYSQLI_ASSOC);
        ...
        $result=array_merge($result,$invoice_line );
        echo( json_encode($result) );
        return;
    }

    $query = 'SELECT * FROM `invoice_lines`';
    $path = explode( "/invoice-lines/", utf8_decode(urldecode($_SERVER['REQUEST_URI'])) );
    ...
    echo( json_encode( mysqli_fetch_all($invoice_lines,MYSQLI_ASSOC) ) );

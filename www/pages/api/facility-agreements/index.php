<?php
    ...
    // S'il n'y a qu'un seul facility_agreement recherché.
    if( isset($url[1]) && !empty($url[1])){
        $query = "SELECT * FROM `facility_agreements` WHERE `facility-agreement-id` = '".$url[1]."' LIMIT 1;";
        ...
        $facility_agreement = mysqli_fetch_array($facility_agreements,MYSQLI_ASSOC);

        if (empty($facility_agreement)) {
            header("HTTP/1.1 404 Not Found");
            $result=array_merge($result,array("success" => false ) );
            $result=array_merge($result,array("message" => "Unknown facility agreement in DB" ) );
        ...
        $result=array_merge($result,$facility_agreement );
        echo( json_encode($result) );
        return;
    }

    $query = 'SELECT * FROM `facility_agreements`';
    $path = explode( "/facility-agreements/", utf8_decode(urldecode($_SERVER['REQUEST_URI'])) );
    ...
    echo( json_encode( mysqli_fetch_all($facility_agreements,MYSQLI_ASSOC) ));

<?php
    ...
    if( isset($url[1]) && !empty($url[1])){
        $query = "SELECT * FROM `credit_notes` WHERE `invoice-line-id` = '".$url[1]."' LIMIT 1;";
        ...
        $credit_notes = mysqli_query($conn, $query);
        $credit_notes = mysqli_fetch_array($credit_notes,MYSQLI_ASSOC);

        if( empty($credit_notes) ) {
            header("HTTP/1.1 404 Not Found");
            $result=array_merge($result,array("success" => false ) );
            $result=array_merge($result,array("message" => "Unknown invoice line in DB" ) );
        ...
        $result=array_merge($result,$credit_notes );
        echo( json_encode($result) );
        return;
    }

    $query = "SELECT * FROM `credit_notes`";
    $path = explode( "/credit-notes/", utf8_decode(urldecode($_SERVER['REQUEST_URI'])) );
    ...
    echo( json_encode( mysqli_fetch_all($credit_notes,MYSQLI_ASSOC) ) );
